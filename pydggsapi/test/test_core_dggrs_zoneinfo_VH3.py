...
def test_core_dggs_zoneinfo_VH3():
    os.environ['dggs_api_config'] = './dggs_api_config.json'
    app = reload(pydggsapi.api).app
    client = TestClient(app)
    print(f"Success test case with dggs zone info (VH3 {cellids[0]})")
    response = client.get(f'/dggs-api/v1-pre/dggs/VH3/zones/{cellids[0]}')
