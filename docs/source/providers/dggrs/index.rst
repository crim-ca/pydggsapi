...
The main idea behind achieving the purpose is straightforward: give a list of original zone IDs, and map the list to the corresponding target zone IDs. So, the return of the convert function has to ensure that the length of the original zone ID list is the same as the converted one.
